...
/**
 * Update a student's profile in Firebase Realtime Database.
 * Only non-sensitive fields (e.g. hostelRoom, phone, fullName) should be updated here.
 */
export async function updateStudentProfile(studentId, newData) {
  if (!studentId) {
    throw new Error('updateStudentProfile: studentId is required');
  }

  // If Firebase is configured, persist changes.
  if (window.firebase && typeof window.firebase.database === 'function') {
    const db = window.firebase.database();
    await db.ref('students').child(studentId).update({
      fullName: newData.fullName,
      hostelRoom: newData.hostelRoom,
      phone: newData.phone,
    });
  } else {
    console.warn('updateStudentProfile: Firebase not configured, running in mock mode.', newData);
  }
}
